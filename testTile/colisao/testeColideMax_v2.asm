...
.text
	...
	CIMA:
		...
		addi $s2, $s1, -1 #posição de um tile acima

		add $a1, $zero, $s0
		add $a2, $zero, $s2
		la $a0, MBG_1
		lw $a0, 0($a0)
		jal getTileInfo #pega informação da posição acima
		
		li $t0, 0x2A
		bne $v0, $t0, maxMove #se a posição acima não for caminho pisável, volte pro loop
		
		#printa tile do BG
		#cima
		la $a0, BG_1
		lw $a0, 0($a0) #carrega endereço do bg_1
		addi $t0, $s1, -1 #pega um tile acima
		move $a1, $zero #zera $a1 por garantia
		add $a1, $s0, $zero #coloca x em $a1
    		sll $a1, $a1, 16 # coloca x na posição certa
  	  	add $a1, $a1, $t0 # coloca y (que está uma posição acima do pé) em |x - y|
		jal printTile
		...
		add $a1, $s0, $zero #coloca x em $a1
    		sll $a1, $a1, 16 #coloca x na posição certa
  	  	add $a1, $a1, $s1 #coloca y (que está no pé) em |x - y|
		jal printTile
		
		
		#printa Max na posição acima
		move $s1, $s2 #carrega a nova posição do pé
		la $a0, MAX_FRONT
		lw $a0, 0($a0)
		add $a1, $zero, $s0 #coloca x
		sll $a1, $a1, 16 #coloca x na posição certa
		addi $t1, $s1, -1 #para printar o max, o y tem que ser -1 porque Max tem 2 tiles, começando do cabelo
		add $a1, $a1, $t1
		jal printInTile
