...
inputManagerProcessBuffer_firstByte:
    ...
    li $a1, 1
    jal inputManagerSetButton # Seta esse byte pra true
    # Vai pro segundo byte
inputManagerProcessBuffer_secondByte:
    sll $a0, $s0, 8
    srl $a0, $a0, 24
    beq $a0, $s2, inputManagerProcessBuffer_firstByteToFalse # Se esse byte for 0x00F0 seta o anterior pra false
    # Se o codigo do segundo byte nao for 0x00F0 seta esse byte pra true se for botao
    li $a1, 1
    jal inputManagerSetButton
    j inputManagerProcessBuffer_thirdByte
inputManagerProcessBuffer_firstByteToFalse:
    srl $a0, $s0, 24
    li $a1, 0
    jal inputManagerSetButton
inputManagerProcessBuffer_thirdByte:
    sll $a0, $s0, 16
    srl $a0, $a0, 24
    beq $a0, $s2, inputManagerProcessBuffer_secondByteToFalse # Se esse byte for 0x00F0 seta o anterior pra false
    # Se o codigo do terceiro byte nao for 0x00F0 seta esse byte pra true se for botao
    li $a1, 1
    jal inputManagerSetButton
    j inputManagerProcessBuffer_fourthByte
inputManagerProcessBuffer_secondByteToFalse:
    sll $a0, $s0, 8
    srl $a0, $a0, 24
    li $a1, 0
    jal inputManagerSetButton
inputManagerProcessBuffer_fourthByte:
    sll $a0, $s0, 24
    srl $a0, $a0, 24
    beq $a0, $s2, inputManagerProcessBuffer_thirdByteToFalse # Se esse byte for 0x00F0 seta o anterior pra false
    # Se o codigo do quarto byte nao for 0x00F0 seta esse byte pra true se for botao
    li $a1, 1
    jal inputManagerSetButton
    j inputManagerProcessBuffer_fifthByte
...
inputManagerProcessBuffer_fifthByte:
    srl $a0, $s1, 24
    beq $a0, $s2, inputManagerProcessBuffer_fourthByteToFalse # Se esse byte for 0x00F0 seta o anterior pra false
    # Se o codigo do quinto byte nao for 0x00F0 seta esse byte pra true se for botao
    li $a1, 1
    jal inputManagerSetButton
    j inputManagerProcessBuffer_sixthByte
...
inputManagerProcessBuffer_sixthByte:
    sll $a0, $s1, 8
    srl $a0, $a0, 24
    beq $a0, $s2, inputManagerProcessBuffer_fifthByteToFalse # Se esse byte for 0x00F0 seta o anterior pra false
    # Se o codigo do quinto byte nao for 0x00F0 seta esse byte pra true se for botao
    li $a1, 1
    jal inputManagerSetButton
    j inputManagerProcessBuffer_seventhByte
...
inputManagerProcessBuffer_seventhByte:
    sll $a0, $s1, 16
    srl $a0, $a0, 24
    beq $a0, $s2, inputManagerProcessBuffer_sixthByteToFalse # Se esse byte for 0x00F0 seta o anterior pra false
    # Se o codigo do quinto byte nao for 0x00F0 seta esse byte pra true se for botao
    li $a1, 1
    jal inputManagerSetButton
...
inputManagerProcessBuffer_eighthByte:
    sll $a0, $s1, 24
    srl $a0, $a0, 24
    beq $a0, $s2, inputManagerProcessBuffer_seventhByteToFalse # Se esse byte for 0x00F0 seta o anterior pra false
    # Se o codigo do quinto byte nao for 0x00F0 seta esse byte pra true se for botao
    li $a1, 1
    jal inputManagerSetButton
    j inputManagerProcessBuffer_end
